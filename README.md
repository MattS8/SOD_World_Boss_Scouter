...
const { creatorId, startTime, boss, discordMsgId, error} = StartLootSession(command.user.id, 'Azuregos')
if (error) { 
    return console.error(error.msg)
}
CurrentLootSessions.put(creatorId, {id : creatorId, startTime : startTime, bossName : boss, msgId: discordMsgId})
```
...
if (result === "AccessDenied")
    return console.error(`User ${command.user.id} does not have permission to change keyword!`)
```
...
#### Example
```js
const result = ForceBeginScouting(command.user.id, GetUserIdFromDisplayName('Beverice'))
