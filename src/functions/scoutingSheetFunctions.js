...
function addBossTimesToRow(scoutObj, headerRow, addedBossTimes) {
  for (key of Object.keys(Config.Scouting)) {
    ...
    scoutObj[scoutArea.name] = (Number(scoutObj[scoutArea.name]) + Number(addedBossTimes[scoutArea.name] || 0) / (60 * 60)).toFixed(4);
  }
...
async function addNewScoutMemberRow(
  ...
  const newRow = addBossTimesToRow(Utils.convertSheetDataToScoutData([username, displayName, id], headerRow), headerRow, startingBossHours);

  // Create SUM cell
  newRow[TotalHoursIndex] = `=SUM(${Constants.ScoutingSheet.bossHoursStartCol}${newRowNumber}:${Constants.ScoutingSheet.bossHoursEndCol}${newRowNumber})`;
...
async function updateScout(user, memberName, guildTag, addedBossTimes) {
  ...
  if (!scoutData || scoutData.length === 0) {
    ...
    scoutObj[TotalHoursIndex] = `=SUM(${Constants.ScoutingSheet.bossHoursStartCol}${changedRow}:${Constants.ScoutingSheet.bossHoursEndCol}${changedRow})`
...
module.exports = (DiscordClient) => {
  DiscordClient.GoogleSheetFunctions.Scouting = {
